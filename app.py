...
st.set_page_config(page_title="ระบบครูตระกูล v8.9", layout="wide")
...
<div class="teacher-card">
    ...
    <div>
        ...
        <p style="margin:0; font-size: 1.2rem; color: #333;">V8.9 | รองรับไฟล์รายชื่อหลายไฟล์ (Excel) | ลำดับที่ | เลขที่ | ส่วน</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_m:
    st.subheader("1️⃣ ไฟล์รายชื่อนักเรียน (อัปได้หลายไฟล์)")
    master_files = st.file_uploader("อัปโหลดไฟล์รายชื่อ (Excel/CSV) - เลือกได้หลายห้องพร้อมกัน", type=["xlsx", "csv"], accept_multiple_files=True)

with col_p:
    st.subheader("2️⃣ ไฟล์ส่งงาน (จาก Padlet)")
    padlet_files = st.file_uploader("อัปโหลดไฟล์จาก Padlet (Excel/CSV)", type=["xlsx", "csv"], accept_multiple_files=True)

# --- 3. ประมวลผลรายชื่อ (รวมไฟล์ Master) ---
student_db = {}
if master_files:
    for mf in master_files:
        try:
            m_df = pd.read_csv(mf, encoding='utf-8-sig') if mf.name.endswith('.csv') else pd.read_excel(mf)
            c_sid = next((c for c in m_df.columns if any(k in str(c) for k in ["เลขที่", "No", "id", "SID"])), None)
            c_name = next((c for c in m_df.columns if any(k in str(c) for k in ["ชื่อ", "Name", "นักเรียน"])), None)
            
            if c_sid and c_name:
                for _, r in m_df.iterrows():
                    try:
                        s_id = int(re.sub(r'\D', '', str(r[c_sid])))
                        s_name = str(r[c_name]).strip()
                        student_db[s_id] = s_name 
                    except: continue
        except Exception as e:
            st.error(f"เกิดข้อผิดพลาดในการโหลดไฟล์รายชื่อ {mf.name}: {e}")
    if student_db:
        st.success(f"✅ รวมฐานข้อมูลรายชื่อสำเร็จ ทั้งหมด {len(student_db)} คน")

# --- 4. ประมวลผลไฟล์ Padlet ---
if padlet_files:
    all_data = []
    ...
    for f in padlet_files:
        try:
            ...
            col_sec = next((c for c in p_df.columns if "ส่วน" in str(c) or "Section" in str(c)), None)
            ...
            for _, row in p_df.iterrows():
                ...
                if sid_match and act_match:
                    ...
                    final_name = student_db.get(sid, f"⚠️ ไม่พบรายชื่อ (เลขที่ {sid})")
